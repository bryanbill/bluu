...
//This Function saves the chosen Theme Color in SharedPreferences.
void showSettingsBottomSheetCanvas(context, canvasList) {
  showModalBottomSheet(
    context: context,
    builder: (BuildContext context) {
      return StreamBuilder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return Row(
                        children: <Widget>[
                          InkWell(
                            child: ColorRadio(
                              canvasList[index].isActive,
                              canvasList[index].colorName,
                              canvasList[index].radioColor,
                            ),
                            onTap: () {
                              if (canvasList[index].isActive) {
                                print('Already selected theme pressed');
                              } else {
                                canvasList.forEach(
                                    (element) => element.isActive = false);
                                ...
                                canvasList[index].isActive =
                                    (snapshot.data == 'true') ? true : false;
                                bloc.sinkCanvas(canvasList[index].colorName);
                                commitCanvas(canvasList[index].colorName);
                              }
                            },
                          ),
                          SizedBox(
                              width: (index == canvasList.length - 1)
                                  ? 0
                                  : ((MediaQuery.of(context).size.width -
                                          (70 + (50 * canvasList.length))) /
                                      (canvasList.length - 1))),
                        ],
                      );
                    },
